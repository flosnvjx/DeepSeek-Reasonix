...
func TestIsReadOnlyBashSubject(t *testing.T) {
	tests := []struct {
		...
		// Read-only commands
		{"ls", true},
		{"ls /data/data/com.termux/files/usr/tmp", true},
		{"cat main.go", true},
		...
		{"echo hello", true},
		{"pwd", true},
		{"cd /data/data/com.termux/files/usr/tmp/project", true},
		{"whoami", true},
		{"wc -l file.txt", true},
		{"stat main.go", true},
		{"du -sh .", true},
		...
		{"cd /data/data/com.termux/files/usr/tmp && rm file.txt", false},
		{"go build ./...", false},
		{"go fmt ./...", false},
		{"go test ./...", false},
		...
		{"awk 'BEGIN{system(\"touch /data/data/com.termux/files/usr/tmp/reasonix-awk-test\")}'", false},
		{"awk 'BEGIN{print \"evil\" > \"/data/data/com.termux/files/usr/tmp/reasonix-awk-test\"}'", false},
		{"sed 's/a/b/' file.txt", false},
		{"sed -e 's/.*/touch \\/data/data/com.termux/files/usr/tmp\\/reasonix-sed-test/e' file.txt", false},
...
func TestBashDangerWarning(t *testing.T) {
	tests := []struct {
		...
		{"ls", ""},
		{"cat main.go", ""},
		{"rm -rf /data/data/com.termux/files/usr/tmp/build", "recursive delete"},
